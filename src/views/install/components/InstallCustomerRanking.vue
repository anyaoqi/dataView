...
<template>
  <div class="rate-ranking">
    ...
    <swiper
      ...
      direction="vertical"
      :loop="false"
      :autoplay="autoplayOptionsVInstallRanking"
      ...
      <swiper-slide v-for="(item, index) in dataList" :key="index" class="item">
        ...
        <div class="progress">
          <a-progress
            ...
            :percent="item.install_rate"
            trailColor="rgba(10, 81, 249, 0.125)"
            strokeLinecap="square"
            :showInfo="false"
          />
        </div>
        <p class="rate">{{ item.install_customer_num }}户</p>
      </swiper-slide>
    </swiper>
  </div>
</template>
...
const autoplayOptionsVInstallRanking = reactive({
  delay: 1000,
...
const pageSize = 10
// 获取安装客户
function getInstallCustomerRankingData() {
  installApi
    .getInstallCustomerRankingData({
      page: page.value,
      pageSize: pageSize,
    })
    .then((res) => {
      ...
      dataList.value.forEach((element) => {
        ...
        element.install_rate =
          (Number(element.install_customer_num) /
            Number(element.belong_customer_num)) *
          100
      })
  ...
  .swiper {
    ...
    .item {
      ...
      .rank-num {
        ...
        line-height: 26px;
        position: relative;
        background: url(../../../assets/images/install/num_bg_blue.png)
          no-repeat;
